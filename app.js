...
const QUESTIONS = [
  ['群里冷场了，你更可能：', ['先看，等别人开口', '找个角度把话题重新盘起来', '直接丢个东西把场面拉热']],
  ['明明你最懂，但现场有人开始装懂，你更像：', ['懒得争，先记着', '找个不难看的方式把话接回来', '直接让大家知道到底谁懂']],
  ['一个重要机会终于轮到你时，你更接近：', ['先确认值不值得我真上桌', '稳住，别飘，把位置坐实', '终于轮到我了，那我这次要狠狠干']],
  ['朋友临时放你鸽子，你第一反应更像：', ['算了，下次再说', '我会记住，但先不说破', '这事我得让对方知道不太行']],
  ['一个项目已经乱成一锅粥了，你通常会：', ['先把最乱的地方稳住', '想办法把局重新盘活', '关键节点直接接管']],
  ['别人对你的第一印象更可能是：', ['这人看起来不简单', '这人应该挺会来事', '这人不一定吵，但感觉有后手']],
  ['需要做一个长期又麻烦的事时，你更像：', ['一旦认真就会一直往前推', '前面一般，后面越来越猛', '先看有没有更聪明的打法']],
  ['一桌人都开始瞎聊，场面有点散，你更可能：', ['不急，先观察局势', '找机会把话题拢回来', '直接主导一个新方向']],
  ['别人抢你功劳时，你更像：', ['先不发作，后面找机会翻回来', '当场把自己的位置拿回来', '不吵，但会让对方知道边界在哪']],
  ['遇到复杂局面时，你的优势更像：', ['我看得比别人早', '我处理得比别人稳', '我进场后局面会变得更可控']],
  ['如果你在一个团队里最常被需要，通常是因为：', ['我能把事情稳住', '我能把烂局救活', '我比较全面，哪都能补']],
  ['你更像哪种“强”：', ['关键时刻特别顶', '长期看特别顶', '一直都挺顶，只是不一定吵']],
  ['大家都上头的时候，你更可能：', ['先看透这局到底怎么回事', '先别急，想后果', '先把最容易炸的地方按住']],
  ['你被夸“厉害”时，更像哪种厉害：', ['自带点不好替代的感觉', '做事强但人也挺鲜活', '执行力一开就停不下来']],
  ['如果你去参加一个陌生局，你更像：', ['不一定多说，但存在感会先到', '很快就能把气氛接住', '先安静一下，看清再出手']],
  ['事情快输了的时候，你更可能：', ['想办法反杀', '想办法兜底', '想办法把局势盘活']],
  ['别人觉得你“有点贵”时，通常指的是：', ['你有稀缺感，不太像普通人', '你气场压人，不太好惹', '你位置感很强，像迟早会上桌的人']],
  ['真到需要拍板的时候，你更像：', ['一直没怎么说话，但一开口就定了', '平时就在慢慢控着节奏', '看起来不显山露水，但其实已经想好后手']],
  ['你最常赢在：', ['分寸感', '气场', '后劲']],
  ['你最容易被误解成：', ['想太多', '有点冷', '平时没怎么发力']],
  ['如果有人说你“难搞”，大概率是因为：', ['你边界硬', '你太清醒', '你总能看穿别人的路数']],
  ['你更愿意别人怎么记住你：', ['关键时候是真的顶', '跟我合作久了就知道我多值', '我不一定最吵，但我很难被替代']],
  ['你在关系和合作里最常做的是：', ['接住场面', '稳住节奏', '让事情继续往前走']],
  ['如果非要说一句最像你的人设，你更可能选：', ['我这种人出手不多，但出手一般都挺重', '我这种人平时不一定显眼，但时间越长越看得出东西', '我这种人不是非要抢风头，但局面经常会往我这里靠']]
].map((item, index) => ({
  id: index + 1,
  text: item[0],
...
const WEIGHTS = {
  ...
  10: { A: { READ: 2, WAKE: 1 }, B: { ANCH: 2, FULL: 1 }, C: { CTRL: 2, ULTI: 1 } },
  11: { A: { ANCH: 2, ULTI: 1 }, B: { SAVE: 2, LIVE: 1 }, C: { FULL: 2, LIVE: 1 } },
  12: { A: { ULTI: 2, CLUT: 1 }, B: { LAST: 2, BURN: 1 }, C: { FULL: 2, LIVE: 1 } },
  ...
  20: { A: { READ: 2, WAKE: 1 }, B: { AURA: 2, RICH: 1 }, C: { LAST: 2, ULTI: 1 } },
  21: { A: { HARD: 2, AURA: 1 }, B: { WAKE: 2, READ: 1 }, C: { READ: 2, HOLD: 1 } },
  22: { A: { ULTI: 2, HARD: 1 }, B: { LAST: 2, FULL: 1 }, C: { FULL: 2, RICH: 1 } },
  23: { A: { SAVE: 2, CTRL: 1 }, B: { ANCH: 2, HOLD: 1 }, C: { BURN: 2, LIVE: 1 } },
  24: { A: { HARD: 2, ULTI: 1 }, B: { LAST: 2, FULL: 1 }, C: { CTRL: 2, AURA: 1 } },
};
